[members] \
Benjamin Gielczynski, Benjamin.Gielczynski@b-tu.de, \
David von Rueden, David.vonRueden@b-tu.de, \
Tim Giese, Tim.Giese@b-tu.de, \
Victor Domke, domkevic@b-tu.de, \
Simon Brokhoff, Simon.Brokhoff@b-tu.de

[Organisatorisches]
python scripts + Presentation 1 day in advance (einen Tag vor 04.06.2026) per github (or email as archive file, zip, not python!)
3 Points für Presentation der Ergebnisse -- Team
2 Points für fachliche Diskussion -- Individual
NO HARDCODED FILE PATHS
Code als notebook oder slides; Muss schnell präsentiert werden.
Naming conventions siehe unten

[Aufnahme]
 Sensoren: 1 Sender und 2 Empfänger
 Proben:   2 Zahnräder vom Typ 2057 Kettenrad (1 gute, 1 mit Fehler)

[Dateinamen]
 Z0(1,5)_Pos01_AAAAA_NNNN/
     Z0(1,5)_Pos01_AAAAA_NNNN_TTTTTTTTTT/
         Wav/
             Z0(1,5)_Pos01_AAAAA_NNNN_TTTTTTTTTT_IIIII_14_(Ch1,Ch2,Wav3).wav

 Pos01: Auflegeposition (beginnend gegenüber Verdickung, gegen Uhrzeigersinn im Abstand von 4 Zähnen)

 AAAAA: Anregungsfunktion:
    Crp1k_200k: Chrip von 1 kHz bis 200 kHz

 NNNN: Messungs-ID

 TTTTTTTTTT: Zeitstempel

 IIIII: Aufnahme-ID

 Ch1,Ch2: Empfängerkanäle
 Wav3:    Sendesignal
